...
static inline bool_t fb_is_argb4444(fb_info_t* fb) {
  struct fb_var_screeninfo* var = &(fb->var);
  if (var->bits_per_pixel == 16 && var->blue.offset == 0 && var->green.offset == 4 &&
      var->red.offset == 8 && var->red.length == 4 && var->green.length == 4 &&
      var->blue.length == 4) {
    return TRUE;
  } else {
    return FALSE;
...
static inline int fb_open(fb_info_t* fb, const char* filename, int w, int h) {
...
#ifdef FB_TDE_ENABLE
  fb->var.xres = ALIGN(w, 16);
  fb->var.yres = ALIGN(h, 16);
  fb->var.xres_virtual = ALIGN(w, 16);
  fb->var.yres_virtual = ALIGN(h, 16) * FB_BUFFER;
#else
  fb->var.xres = w;
  fb->var.yres = h;
  fb->var.xres_virtual = ALIGN(w,8);
  fb->var.yres_virtual = h * FB_BUFFER;
#endif

  fb->var.bits_per_pixel = FB_DEPTH;

  if (strcmp(BOARD, "CV184XH") == 0) {
    if (strcmp(FB_FORMAT, "ARGB4444") == 0) {
      fb->var.transp.offset = 12; fb->var.transp.length = 4; fb->var.transp.msb_right = 0;
      fb->var.red.offset = 8;     fb->var.red.length = 4;    fb->var.red.msb_right = 0;
      fb->var.green.offset = 4;   fb->var.green.length = 4;  fb->var.green.msb_right = 0;
      fb->var.blue.offset = 0;    fb->var.blue.length = 4;   fb->var.blue.msb_right = 0;
    } else {
      fb->var.transp.offset = 24; fb->var.transp.length = 8; fb->var.transp.msb_right = 0;
      fb->var.red.offset = 16;    fb->var.red.length = 8;    fb->var.red.msb_right = 0;
      fb->var.green.offset = 8;   fb->var.green.length = 8;  fb->var.green.msb_right = 0;
      fb->var.blue.offset = 0;    fb->var.blue.length = 8;   fb->var.blue.msb_right = 0;
    }
  }
  // fb->var.green.length = 4;
  ...
  if ((fb_is_argb4444(fb) == FALSE) || (2 == fb_nr)) {
#ifdef FB_TDE_ENABLE
    CVI_TDE_Open();
    CVI_SYS_IonAlloc(&fb->fbmem_phyaddr, &fb->fbmem_offline, "gfbg_tde_buffer", size);
#else
...
#ifdef FB_TDE_ENABLE
      CVI_SYS_IonFree(fb->fbmem_phyaddr, fb->fbmem_offline);
      CVI_TDE_Close();
#else

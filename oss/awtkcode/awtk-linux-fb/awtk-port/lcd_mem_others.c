...
static ret_t lcd_bgra5551_flush(lcd_t* lcd) {
  ...
  uint32_t src_line_length = special->base.line_length/bpp;
  ...
  if (o == LCD_ORIENTATION_0) {
    if (dr->w > 0 && dr->h > 0) {
      ...
      uint16_t* src = (uint16_t*)(special->base.offline_fb);
...
static ret_t lcd_argb4444_flush(lcd_t* lcd) {
  xy_t dx = 0;
  xy_t dy = 0;
  uint32_t i = 0;
  uint32_t k = 0;
  uint32_t w = 0;
  uint32_t h = 0;
  // uint32_t bpp = bitmap_get_bpp_of_format(BITMAP_FMT_ARGB8888);
  uint32_t* src_p = NULL;
  uint16_t* dst_p = NULL;
  ...
  uint32_t dst_line_length = (info->fix.line_length)/bitmap_get_bpp_of_format(BITMAP_FMT_BGR565);
  uint32_t src_line_length = special->base.line_length/bitmap_get_bpp_of_format(BITMAP_FMT_BGRA8888);
  lcd_orientation_t o = system_info()->lcd_orientation;
  if (o == LCD_ORIENTATION_0) {
    if (dr->w > 0 && dr->h > 0) {
      int32_t x = 0;
      ...
      int dst_line_length = info->fix.line_length / 2;
      uint16_t* dst = (uint16_t*)(info->fbmem0);
      uint32_t* src = (uint32_t*)(special->base.offline_fb);
      ...
      for (y = 0; y < dr->h; y++) {
        for (x = 0; x < dr->w; x++) {
          uint32_t s = src[x];
          uint8_t b = (0x000000FF & s) >> 4;
          uint8_t g = (0x0000FF00 & s) >> 12;
          uint8_t r = (0x00FF0000 & s) >> 20;
          uint8_t a = (0xFF000000 & s) >> 28;
          dst[x] = (a << 12) | (r << 8) | (g << 4) | b;
        }
  ...
  uint32_t* src = (uint32_t*)(special->base.offline_fb);
  dst_p = (uint16_t*)(dst + dy * dst_line_length + dx);
  src_p = (uint32_t*)(src + dr->y * src_line_length + dr->x);
  ...
  switch (o) {
    case LCD_ORIENTATION_90: {
      for (i = 0; i < h; i++) {
        ...
        dst_p++;
        src_p = (uint32_t*)(src_p + src_line_length);
      }
      break;
    }
    case LCD_ORIENTATION_180: {
    ...
    case LCD_ORIENTATION_270: {
      ...
      csi_2d_rgb32_to_argb4444_with_rotate_270(dst_p - h, src_p, (dst_line_length*2), (src_line_length*4), w, h);
#else
      for (i = 0; i < h; i++) {
        uint32_t* s = src_p;
        uint16_t* d = dst_p;
...
lcd_t* lcd_mem_argb4444_create(fb_info_t* info) {
  wh_t w = fb_width(info);
  wh_t h = fb_height(info);

  return lcd_mem_special_create(w, h, BITMAP_FMT_BGRA8888, lcd_argb4444_flush, NULL, NULL, info);
}

...
ret_t wstr_format_date_time(wstr_t* str, const char* format, const date_time_t* dt) {
  ...
  wstr_t wformat;
  wstr_init(&wformat, strlen(format) + 3);
  wstr_set_utf8(&wformat, format);
  const wchar_t* p = wformat.str;
  bool_t apformat = FALSE;

  if (strchr(format, 'A') != NULL) {
    apformat = TRUE;
  }
  ...
  while (*p) {
    ...
    switch (*p) {
      ...
      case 's': {
        ...
        if (apformat == TRUE) {
          if (dt->hour > 12) {
            const char* utf8 = locale_info_tr(locale_info(), " PM");
            tk_utf8_to_utf16(utf8, temp, ARRAY_SIZE(temp));
          } else {
            const char* utf8 = locale_info_tr(locale_info(), " AM");
            tk_utf8_to_utf16(utf8, temp, ARRAY_SIZE(temp));
          }
          wstr_append(str, temp);
        }

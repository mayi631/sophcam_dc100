...
static const AVOption options[] = {
    ...
    { "flush_moov_stream", "video track", offsetof(MOVMuxContext, flush_moov_stream_id), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, 0 },
    { NULL },
};
...
static void build_chunks(MOVMuxContext *mov, MOVTrack *trk)
{
    int i;
    ...
    trk->chunkCount = 1;
    if ((mov->flags & FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME) && trk->par->codec_type == AVMEDIA_TYPE_VIDEO) {
        chunk->samples_in_chunk = 1;
    }
...
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (par->codec_id == AV_CODEC_ID_H264 && trk->vos_len > 0 && *(uint8_t *)trk->vos_data != 1 && !TAG_IS_AVCI(trk->tag)) {
        ...
        if (trk->cenc.aes_ctr) {
			...
			if(trk->par->codec_type == AVMEDIA_TYPE_AUDIO){
...
static int mov_write_header(AVFormatContext *s)
{
    ...
    if (mov->flags & FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME) {
        ...
        int remain_align_size = align_size - avio_tell(pb);
        avio_wb32(pb, remain_align_size);
        ffio_wfourcc(pb, "free");
        ffio_fill(pb, 0, remain_align_size - 8);
        avio_seek(pb, align_size, SEEK_SET);
    }

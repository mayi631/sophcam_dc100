...
static int write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (mov->flags & FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME && mov->reserved_moov_size > 0 && pkt->flags & AV_PKT_FLAG_KEY && (pkt->stream_index == mov->flush_moov_stream_id)) {
        AVIOContext *pb = s->pb;
        ...
        int64_t moov_size = mov_write_moov_tag(pb, mov, s);
        int64_t align = (moov_size + 4095) & ~4095; // align 4k
        int64_t remain_size = align - moov_size;
        if (remain_size < 8) {
            remain_size += 4096;
        }
        avio_wb32(pb, remain_size);
        ffio_wfourcc(pb, "free");
        ffio_fill(pb, 0, remain_size - 8);

        flush_if_needed(s);
        avio_seek(pb, mdat_pos, SEEK_SET);
    }
    return ret;
}

...
#if HAVE_PREALLOC
    { "preallocsize", "set prealloc block size", offsetof(FileContext, presize), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM },
#endif
    { NULL }
};
...
extern int fallocate (int __fd, int __mode, off_t __offset, off_t __len);

static int file_fallocate(URLContext *h)
{
    FileContext *c = h->priv_data;
    if(c->presize > 0){
        // posix_fallocate(c->fd, (c->alloccnt * (c->presize << 20)), c->presize << 20);
        fallocate(c->fd, FALLOC_FL_KEEP_SIZE, c->alloccnt * (c->presize << 20), c->presize << 20);
        c->alloccnt++;
    }
    return 0;
}

static int file_fix_tail(URLContext *h)
{
    FileContext *c = h->priv_data;
    char *suffix = strrchr(h->filename, '.');
    if(suffix && strncasecmp(suffix + 1, "ts", 2) == 0){
        ftruncate(c->fd, c->wpos);
        return 0;
    }

    int remain = (c->alloccnt * (c->presize << 20)) - c->wpos;
    if(remain > 0){
        char buf[8] = {0};
        int len = (remain > sizeof(buf)?sizeof(buf):remain);
        write(c->fd, buf, len);
        c->wpos += len;
        ftruncate(c->fd, c->alloccnt * (c->presize << 20));
    }
    return 0;
}
#endif
...
static int file_write(URLContext *h, const unsigned char *buf, int size)
{
    ...
    // size = FFMIN(size, c->blocksize);
#if HAVE_PREALLOC
    if(c->presize > 0 && c->wpos + size >= (c->alloccnt * (c->presize << 20))){
        file_fallocate(h);
    }
#endif
...
static int file_open(URLContext *h, const char *filename, int flags)
{
    ...
    if (!h->is_streamed && flags & AVIO_FLAG_WRITE)
        h->min_packet_size = h->max_packet_size = 256 << 10;

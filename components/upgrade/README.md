...
upgrade lib will compare **ModelName** and **SoftVer** between `UPGRADE_DEV_INFO_S` and package header to decide if upgrade allow-able.

**ModelName** **MUST** be the same and **SoftVer** comparsion according `strcmp()` rule with a flag, which able to disable check.

Currently, `sample_upgrade` **DOES NOT** do version check; modify 3rd argument of `UPGRADE_CheckPkg()` to enable it.

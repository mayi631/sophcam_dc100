...
{
	return; /** 8188 no need power on reset*/
}

void WIFI_HAL_SysPreInit(void)
{
	static bool bPinOutInit = false;
	OSAL_FS_System("echo host > /proc/cviusb/otg_role");
	HAL_GPIO_Set_Value(HAL_GPIOE_18, HAL_GPIO_VALUE_L);
	cvi_usleep(10 * 1000);
	HAL_GPIO_Set_Value(HAL_GPIOE_18, HAL_GPIO_VALUE_H);
	if (true == bPinOutInit) {
...
int32_t WIFI_HAL_LoadAPDriver(void)
{
	...
	s32Ret = OSAL_FS_Insmod(KOMOD_PATH "/3rd/8188fu.ko", NULL);
	if (0 != s32Ret) {
...
int32_t WIFI_HAL_RemoveDriver(void)
{
	...
	s32Ret = OSAL_FS_Rmmod(KOMOD_PATH "/3rd/8188fu.ko");
	if (0 != s32Ret) {
		printf("insmod ap: failed, errno(%d)\n", errno);
		return -1;
	}

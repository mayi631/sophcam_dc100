...
{
	HAL_GPIO_Set_Value(HAL_GPIOB_11, HAL_GPIO_VALUE_H);
...
int32_t WIFI_HAL_LoadAPDriver(void)
{
	...
	s32Ret = OSAL_FS_Insmod(KOMOD_PATH "/3rd/aic8800_bsp.ko", "aic_fw_path=/mnt/data/bin/aic8800");
	// s32Ret |= OSAL_FS_Insmod(KOMOD_PATH "/aic8800_btlpm.ko",NULL);
	s32Ret |= OSAL_FS_Insmod(KOMOD_PATH "/3rd/aic8800_fdrv.ko", NULL);
	if (0 != s32Ret) {
...
{
	int32_t s32Ret = 0;
	s32Ret |= OSAL_FS_Rmmod(KOMOD_PATH "/3rd/aic8800_fdrv.ko");
	s32Ret |= OSAL_FS_Rmmod(KOMOD_PATH "/3rd/aic8800_bsp.ko");
	if (0 != s32Ret) {
		printf("rmmod ap: failed, errno(%d)\n", errno);
		return -1;
	}
	return 0;
}

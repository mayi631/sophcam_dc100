...
static HAL_WIFI_MODE_E s_enHALWIFIMode = HAL_WIFI_MODE_BUIT;
static bool s_bHALWIFIStartState = false;
static pthread_mutex_t sWifiMutex = PTHREAD_MUTEX_INITIALIZER;

static int32_t HAL_WIFI_ApValidChck(const HAL_WIFI_APMODE_CFG_S* pstApCfg)
{
    /** cahnnel check */
...
static int32_t HAL_WIFI_StaValidChck(const HAL_WIFI_STAMODE_CFG_S* pstStaCfg)
{
    printf("[Errot]wifi STA mode parm validcheck not support %d", pstStaCfg->enStaMode);
    return -1;
}

static int32_t HAL_WIFI_InParmValidChck(const HAL_WIFI_CFG_S* pstCfg)
{
    int32_t s32Ret = 0;
...
int32_t HAL_WIFI_Start(const HAL_WIFI_CFG_S* pstCfg)
{
    ...
    switch (s_enHALWIFIMode)
    {
        case HAL_WIFI_MODE_STA:
            ...
            break;
        case HAL_WIFI_MODE_AP:
            s32Ret = WIFI_AP_Start(HAL_WIFI_INTERFACE_NAME,&pstCfg->unCfg.stApCfg);
            if(0 == s32Ret)
            {
...
int32_t HAL_WIFI_Deinit(void)
{
    ...
    s32Ret = WIFI_HAL_RemoveDriver();
    if(0 == s32Ret)
    {
        s_enHALWIFIMode = HAL_WIFI_MODE_BUIT;

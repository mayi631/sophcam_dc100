...
{
	HAL_GPIO_Set_Value(HAL_GPIOE_00, HAL_GPIO_VALUE_L);
	cvi_usleep(10 * 1000);
	HAL_GPIO_Set_Value(HAL_GPIOE_00, HAL_GPIO_VALUE_H);
...
int32_t WIFI_HAL_LoadAPDriver(void)
{
	...
	s32Ret = OSAL_FS_Insmod(KOMOD_PATH "/3rd/hi3881.ko", NULL);
	if (0 != s32Ret) {
...
int32_t WIFI_HAL_RemoveDriver(void)
{
	...
	s32Ret = OSAL_FS_Rmmod(KOMOD_PATH "/3rd/hi3881.ko");
	if (0 != s32Ret) {
		printf("insmod ap: failed, errno(%d)\n", errno);
		return -1;
	}

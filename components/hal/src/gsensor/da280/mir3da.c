...
int32_t mir3da_register_read(unsigned char addr, unsigned char *data)
{
	int32_t res = 0;
...
int32_t mir3da_register_write(unsigned char addr, unsigned char data)
{
	int32_t res = 0;
...
int32_t mir3da_register_read_continuously(unsigned char addr, unsigned char count,
				      unsigned char *data)
{
	int32_t res = 0;
...
int32_t mir3da_register_mask_write(unsigned char addr, unsigned char mask,
			       unsigned char data)
{
	int32_t res = 0;
...
static int32_t mir3da_set_enable(char enable)
{
	int32_t res = 0;
	if (enable)
...
static int32_t mir3da_close_interrupt(void)
{
	int32_t res = 0;
...
static int32_t mir3da_open_interrupt(int32_t num)
{
	int32_t res = 0;
	// unsigned char data;
	res |= mir3da_register_write(NSA_REG_INT_PIN_CONFIG, 0x01);
	res |= mir3da_register_write(NSA_REG_INTERRUPT_SETTINGS1,0x03);//0x07);//
	res |= mir3da_register_write(NSA_REG_INTERRUPT_SETTINGS2,0xA0); // interuppt plus  keep 1 second
	res |= mir3da_register_write(NSA_REG_INT_LATCH, 0x06);
	res |= mir3da_register_write(NSA_REG_ACTIVE_DURATION,0x01);// 0x03
	res |= mir3da_register_write(NSA_REG_ACTIVE_THRESHOLD,0x14);//0x05//0x26////DEBUG//38//DF
	...
	printf("------mir3da_open_interrupt-----\r\n");
	// res = mir3da_register_read(NSA_REG_INTERRUPT_SETTINGS1, &data);
	// printf("------mir3da_register_read NSA_REG_INTERRUPT_SETTINGS1 %x-----\r\n", data);
	// res = mir3da_register_read(NSA_REG_INTERRUPT_MAPPING1, &data);
	// printf("------mir3da_register_read NSA_REG_INTERRUPT_MAPPING1 %x-----\r\n", data);
...
int32_t gsensor_init(void)
{
	...
	res |= mir3da_close_interrupt();
	// res |= mir3da_open_interrupt(0);
	mir3da_set_enable(1);
	return res;
}
...
HAL_GSENSOR_OBJ_S gsensorObj = {
	...
	.open_interrupt = mir3da_open_interrupt,

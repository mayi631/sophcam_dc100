...
#define HAL_TOUCHPAD_KO_PATH KOMOD_PATH "/3rd/gt9xx.ko"
...
static int HAL_TOUCHPAD_PushSamp(HAL_TOUCHPAD_MTINPUT_S *pstMtInput, struct input_event *pstInputEvent, HAL_TOUCHPAD_MTSAMPLE_S *pstMtSamp)
{
	...
	for (s32Slot = 0; s32Slot < HAL_TOUCHAPD_MAX_POINT_NUM; s32Slot++) {
		...
		#ifdef USING_ARM_MUSL_TOOLCHAIN
		pstMtSamp[s32Num].tv.tv_sec = pstInputEvent->__sec;
		pstMtSamp[s32Num].tv.tv_usec = pstInputEvent->__usec;
		#else
...
int HAL_TOUCHPAD_Init(void)
{
	...
	if (false == s_bTOUCHPADInitState) {
		...
		s32Ret = OSAL_FS_Insmod(HAL_TOUCHPAD_KO_PATH, NULL);
		if (0 != s32Ret) {
...
int HAL_TOUCHPAD_Start(int *ps32Fd)
{
	...
	*ps32Fd = s_s32TOUCHPADfd;
	...
	if ((0 == HAL_TOUCHPAD_InputBitCheck(ABS_MT_POSITION_X, au32Inputbits)) || (0 == HAL_TOUCHPAD_InputBitCheck(ABS_MT_POSITION_Y, au32Inputbits)) || (0 == HAL_TOUCHPAD_InputBitCheck(ABS_MT_TOUCH_MAJOR, au32Inputbits))) {
...
int HAL_TOUCHPAD_Deinit(void)
{
	...
	s32Ret = OSAL_FS_Rmmod(HAL_TOUCHPAD_KO_PATH);
	if (0 != s32Ret) {

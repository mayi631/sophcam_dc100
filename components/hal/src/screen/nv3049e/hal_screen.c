...
/** \addtogroup 	SCREEN */
/** @{ */  /** <!-- [SCREEN] */
static HAL_SCREEN_STATE_E g_halScreenCtx = HAL_SCREEN_STATE_ON;
...
static int32_t HAL_SCREEN_Init(void)
{
	...
	for (i = GPIO_TYPE_RESET; i < GPIO_TYPE_BUTT; i++) {
		if (screen_gpio[i].gpio != 0) {
			HAL_GPIO_Export(screen_gpio[i].gpio);
			HAL_GPIO_Direction_Output(screen_gpio[i].gpio);
		}
	}
...
static int32_t HAL_SCREEN_SetBackLightState(HAL_SCREEN_STATE_E enBackLightState)
{
	...
	HAL_GPIO_Set_Value(screen_gpio[GPIO_TYPE_POWER].gpio, HAL_GPIO_VALUE_H);

/*
 * Copyright (C) mapi Co., Ltd. 2019-2020. All rights reserved.
 *
 * File Name: audio_aac_adp.c
 * Description:
...
#define SUPPORT_EXTERNAL_AAC
#define MAPI_AUDIO_ASSERT(x) {	\
	if (CVI_TRUE != (x)) \
		return -1;  \
}
...
#define MAPI_ERR_AENC_NOT_SUPPORT       0xA2000008
#define MAPI_ERR_AENC_ILLEGAL_PARAM     0xA2000003
#define MAPI_ERR_ADEC_NOT_SUPPORT       0xA3000008
#define MAPI_ERR_AENC_NULL_PTR         0xA2000006
#define MAPI_ERR_AENC_NOMEM            0xA200000A
#define MAPI_ERR_ADEC_NOMEM             0xA300000A
#define MAPI_ERR_ADEC_DECODER_ERR       0xA300000F
#define MAPI_ERR_ADEC_BUF_LACK          0xA3000010
//# aac enc lib
typedef CVI_S32(*pMAPI_AACENC_GetVersion_Callback)(AACENC_VERSION_S * pVersion);
...
//# aac dec lib
#if 0
typedef CVI_S32(*pMAPI_AACDEC_GetVersion_Callback)(AACDEC_VERSION_S * pVersion);
#else
typedef int(*pMAPI_AACDEC_GetVersion_Callback)(AACDEC_VERSION_S *pVersion);
#endif

typedef MAPIAACDecoder(*pAACInitDecoder_Callback)(AACDECTransportType enTranType);
typedef CVI_VOID(*pAACFreeDecoder_Callback)(MAPIAACDecoder MAPIAACDecoder);
typedef CVI_S32(*pAACSetRawMode_Callback)(MAPIAACDecoder mapiAACDecoder, CVI_S32 nChans, CVI_S32 sampRate);
typedef CVI_S32(*pAACDecodeFindSyncHeader_Callback)(MAPIAACDecoder mapiAACDecoder,
			CVI_U8 **ppInbufPtr,
			CVI_S32 * pBytesLeft);
typedef CVI_S32(*pAACDecodeFrame_Callback)(MAPIAACDecoder mapiAACDecoder,
			CVI_U8 **ppInbufPtr,
			CVI_S32 * pBytesLeft, CVI_S16 * pOutPcm);
typedef CVI_S32(*pAACGetLastFrameInfo_Callback)(MAPIAACDecoder mapiAACDecoder, AACFrameInfo * aacFrameInfo);
typedef CVI_S32(*pAACDecoderSetEosFlag_Callback)(MAPIAACDecoder mapiAACDecoder, CVI_S32 s32Eosflag);
typedef CVI_S32(*pAACFlushCodec_Callback)(MAPIAACDecoder mapiAACDecoder);

typedef struct {
	CVI_S32 s32OpenCnt;
	CVI_VOID *pLibHandle;
	pMAPI_AACENC_GetVersion_Callback pMAPI_AACENC_GetVersion;
...
typedef struct {
	CVI_S32 s32OpenCnt;
	CVI_VOID *pLibHandle;
	pMAPI_AACDEC_GetVersion_Callback pMAPI_AACDEC_GetVersion;
...
static CVI_S32 InitAacAencLib(void)
{
	...
	if (g_stAacEncFunc.s32OpenCnt == 0) {
		...
		s32Ret = MAPI_Audio_Dlopen(&(stAacEncFunc.pLibHandle), AAC_ENC_LIB_NAME);
		if (s32Ret != CVI_SUCCESS) {
			printf("[Func]:%s [Line]:%d [Info]:%s\n",
			       __func__, __LINE__, "load aenc lib fail!\n");
			return MAPI_ERR_AENC_NOT_SUPPORT;
		}

		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacEncFunc.pMAPI_AACENC_GetVersion),
				stAacEncFunc.pLibHandle, "MAPI_AACENC_GetVersion");
		if (s32Ret != CVI_SUCCESS) {
			printf("[Func]:%s [Line]:%d [Info]:%s\n",
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacEncFunc.pAACInitDefaultConfig),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacEncFunc.pAACEncoderOpen),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacEncFunc.pAACEncoderFrame),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacEncFunc.pAACEncoderClose),
...
CVI_VOID DeInitAacAencLib(CVI_VOID)
{
	...
	if (g_stAacEncFunc.s32OpenCnt == 0) {
		if (g_stAacEncFunc.pLibHandle != CVI_NULL)
			MAPI_Audio_Dlclose(g_stAacEncFunc.pLibHandle);
...
CVI_S32 MAPI_AACENC_GetVersion_Adp(AACENC_VERSION_S *pVersion)
{
	if (g_stAacEncFunc.pMAPI_AACENC_GetVersion == CVI_NULL) {
		printf("[Func]:%s [Line]:%d [Info]:%s\n",
		       __func__, __LINE__, "call aac function fail!\n");
		return MAPI_ERR_AENC_NOT_SUPPORT;
	}
	return g_stAacEncFunc.pMAPI_AACENC_GetVersion(pVersion);
}
...
static CVI_S32 InitAacAdecLib(void)
{
	...
	if (g_stAacDecFunc.s32OpenCnt == 0) {
		...
		s32Ret = MAPI_Audio_Dlopen(&(stAacDecFunc.pLibHandle), AAC_DEC_LIB_NAME);
		if (s32Ret != CVI_SUCCESS) {
			printf("[Func]:%s [Line]:%d [Info]:%s\n",
			       __func__, __LINE__, "load aenc lib fail!\n");
			return MAPI_ERR_ADEC_NOT_SUPPORT;
		}

		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pMAPI_AACDEC_GetVersion),
				stAacDecFunc.pLibHandle, "MAPI_AACDEC_GetVersion");
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pAACInitDecoder),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pAACFreeDecoder),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pAACSetRawMode),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pAACDecodeFindSyncHeader),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pAACDecodeFrame),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pAACGetLastFrameInfo),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pAACDecoderSetEosFlag),
		...
		s32Ret = MAPI_Audio_Dlsym((CVI_VOID **)&(stAacDecFunc.pAACFlushCodec),
...
CVI_VOID DeInitAacAdecLib(CVI_VOID)
{
	...
	if (g_stAacDecFunc.s32OpenCnt == 0) {
		if (g_stAacDecFunc.pLibHandle != CVI_NULL)
			MAPI_Audio_Dlclose(g_stAacDecFunc.pLibHandle);
...
CVI_S32 MAPI_AACDEC_GetVersion_Adp(AACDEC_VERSION_S *pVersion)
{
	if (g_stAacDecFunc.pMAPI_AACDEC_GetVersion == CVI_NULL) {
		printf("[Func]:%s [Line]:%d [Info]:%s\n",
		       __func__, __LINE__, "call aac function fail!\n");
		return MAPI_ERR_ADEC_NOT_SUPPORT;
	}

	return g_stAacDecFunc.pMAPI_AACDEC_GetVersion(pVersion);
}

MAPIAACDecoder AACInitDecoder_Adp(AACDECTransportType enTranType)
{
...
CVI_VOID AACFreeDecoder_Adp(MAPIAACDecoder mapiAACDecoder)
{
	...
	return g_stAacDecFunc.pAACFreeDecoder(mapiAACDecoder);
}

CVI_S32 AACSetRawMode_Adp(MAPIAACDecoder mapiAACDecoder, CVI_S32 nChans, CVI_S32 sampRate)
{
	...
	return g_stAacDecFunc.pAACSetRawMode(mapiAACDecoder, nChans, sampRate);
}

CVI_S32 AACDecodeFindSyncHeader_Adp(MAPIAACDecoder mapiAACDecoder, CVI_U8 **ppInbufPtr, CVI_S32 *pBytesLeft)
{
	...
	return g_stAacDecFunc.pAACDecodeFindSyncHeader(mapiAACDecoder, ppInbufPtr, pBytesLeft);
}

CVI_S32 AACDecodeFrame_Adp(MAPIAACDecoder mapiAACDecoder, CVI_U8 **ppInbufPtr, CVI_S32 *pBytesLeft, CVI_S16 *pOutPcm)
{
	...
	return g_stAacDecFunc.pAACDecodeFrame(mapiAACDecoder, ppInbufPtr, pBytesLeft, pOutPcm);
}

CVI_S32 AACGetLastFrameInfo_Adp(MAPIAACDecoder mapiAACDecoder, AACFrameInfo *aacFrameInfo)
{
	...
	return g_stAacDecFunc.pAACGetLastFrameInfo(mapiAACDecoder, aacFrameInfo);
}

CVI_S32 AACDecoderSetEosFlag_Adp(MAPIAACDecoder mapiAACDecoder, CVI_S32 s32Eosflag)
{
	...
	return g_stAacDecFunc.pAACDecoderSetEosFlag(mapiAACDecoder, s32Eosflag);
}

CVI_S32 AACFlushCodec_Adp(MAPIAACDecoder mapiAACDecoder)
{
	...
	return g_stAacDecFunc.pAACFlushCodec(mapiAACDecoder);
}
...
static CVI_S32 AencCheckAACAttr(const AENC_ATTR_AAC_S *pstAACAttr)
{
	if (pstAACAttr->enBitWidth != AUDIO_BIT_WIDTH_16) {
		...
		return MAPI_ERR_AENC_ILLEGAL_PARAM;
	}
...
CVI_S32 OpenAACEncoder(CVI_VOID *pEncoderAttr, CVI_VOID **ppEncoder)
{
	...
	MAPI_AUDIO_ASSERT(pEncoderAttr != CVI_NULL);
	MAPI_AUDIO_ASSERT(ppEncoder != CVI_NULL);
							...
							CVI_S32 s32InputSizeBytes, CVI_U32 *pu32OutLen)
{
	...
	MAPI_AUDIO_ASSERT(pEncoder != CVI_NULL);
	MAPI_AUDIO_ASSERT(inputdata != CVI_NULL);
	MAPI_AUDIO_ASSERT(pu8Outbuf != CVI_NULL);
	MAPI_AUDIO_ASSERT(pu32OutLen != CVI_NULL);
		    ...
		    CVI_U8 *pu8Outbuf, CVI_U32 *pu32OutLen)
{
	...
	MAPI_AUDIO_ASSERT(pEncoder != CVI_NULL);
	MAPI_AUDIO_ASSERT(pstData != CVI_NULL);
	MAPI_AUDIO_ASSERT(pu8Outbuf != CVI_NULL);
	MAPI_AUDIO_ASSERT(pu32OutLen != CVI_NULL);
...
CVI_S32 OpenAACDecoder(CVI_VOID *pDecoderAttr, CVI_VOID **ppDecoder)
{
	...
	MAPI_AUDIO_ASSERT(pDecoderAttr != CVI_NULL);
	MAPI_AUDIO_ASSERT(ppDecoder != CVI_NULL);
	...
	*ppDecoder = (CVI_VOID *)pstDecoder;
	...
	if (!pstDecoder->pstAACState) {
		...
		return MAPI_ERR_ADEC_DECODER_ERR;
	}
	} else {
		    ...
		    CVI_U16 *pu16Outbuf, CVI_U32 *pu32OutLen, CVI_U32 *pu32Chns)
{
	...
	MAPI_AUDIO_ASSERT(pDecoder != CVI_NULL);
	MAPI_AUDIO_ASSERT(pu8Inbuf != CVI_NULL);
	MAPI_AUDIO_ASSERT(ps32LeftByte != CVI_NULL);
	MAPI_AUDIO_ASSERT(pu16Outbuf != CVI_NULL);
	MAPI_AUDIO_ASSERT(pu32OutLen != CVI_NULL);
	MAPI_AUDIO_ASSERT(pu32Chns != CVI_NULL);
...
CVI_S32 GetAACFrmInfo(CVI_VOID *pDecoder, CVI_VOID *pInfo)
{
	...
	MAPI_AUDIO_ASSERT(pDecoder != CVI_NULL);
	MAPI_AUDIO_ASSERT(pInfo != CVI_NULL);
...
CVI_S32 MAPI_MPI_AENC_AacInit(CVI_VOID)
{
	...
	return CVI_SUCCESS;
#else
	printf("[Warning]Not loading external AAC lib to aenc\n");
	return CVI_FAILURE;
...
CVI_S32 MAPI_MPI_AENC_AacDeInit(CVI_VOID)
{
	CVI_S32 s32Ret;
#ifdef SUPPORT_EXTERNAL_AAC
...
CVI_S32 MAPI_MPI_ADEC_AacInit(CVI_VOID)
{
	CVI_S32 s32Handle, s32Ret;
#ifdef SUPPORT_EXTERNAL_AAC
	...
	return CVI_SUCCESS;
#else
	printf("[Warning]Not loading external AAC lib to adec\n");
	return CVI_FAILURE;
...
CVI_S32 MAPI_MPI_ADEC_AacDeInit(CVI_VOID)
{
	CVI_S32 s32Ret;
#ifdef SUPPORT_EXTERNAL_AAC
...
static int _aac_transcode_option(void)
{
	...
	// follow the AENC_ATTR_AAC_S  in audio_aac_adp.h
	printf("\n");

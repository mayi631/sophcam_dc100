 ...
 *N/A
 */
CVI_S32 MAPI_AACDEC_GetVersion(AACDEC_VERSION_S *pVersion)
{
 ...
 *retval ::MAPIAACDecoder   : init success, return non-NULL handle.
 *retval ::NULL          : init failure, return NULL
 *see
 *N/A
 */
MAPIAACDecoder AACInitDecoder(AACDECTransportType enTranType)
{
 ...
 *attention \n
 *N/A
 *param[in] MAPIAACDecoder  : AAC-Decoder handle
 *see \n
 *N/A
 */
CVI_VOID AACFreeDecoder(MAPIAACDecoder MAPIAACDecoder)
{
	if (MAPIAACDecoder != NULL)
		aacDecoder_Close((HANDLE_AACDECODER)MAPIAACDecoder);
 ...
 *N/A
 */
CVI_S32 AACSetRawMode(MAPIAACDecoder MAPIAACDecoder,
							CVI_S32 nChans,
							CVI_S32 sampRate)
{
	...
	err = aacDecoder_ConfigRaw((HANDLE_AACDECODER)MAPIAACDecoder,
    		conf_array,
		&length);
 ...
 *N/A
 */
CVI_S32 AACDecodeFindSyncHeader(MAPIAACDecoder MAPIAACDecoder,
										CVI_U8 **ppInbufPtr,
										CVI_S32 *pBytesLeft)
{
#if 0
...
#endif
#if 0
呼叫 aacDecoder_Fill 把資料填進 internal buffer 裡面, 然後再做解碼.
在此需要注意的是因為 fdk-aac 支援多層, 所以他的讀取是以 array of pointer 去讀取,
那要送進 internal buffer 的時候也是要包裝成這種格式.
#endif
	...
	err = aacDecoder_Fill((HANDLE_AACDECODER)MAPIAACDecoder,
							inBufferArray,
							inBuffReaded,
							&bytes_valid);
 ...
 *see \n
 */
CVI_S32 AACDecodeFrame(MAPIAACDecoder MAPIAACDecoder,
							CVI_U8 **ppInbufPtr,
							CVI_S32 *pBytesLeft,
							CVI_S16 *pOutPcm)
{
	...
	err = aacDecoder_DecodeFrame((HANDLE_AACDECODER)MAPIAACDecoder,
							(INT_PCM *) pOutPcm,
							(1024 * 30), 0);
	...
	theInfo = aacDecoder_GetStreamInfo((HANDLE_AACDECODER)MAPIAACDecoder);
	if (theInfo == NULL) {
 ...
 *N/A
 */
CVI_S32 AACGetLastFrameInfo(MAPIAACDecoder MAPIAACDecoder,
								AACFrameInfo *aacFrameInfo)
{
	HANDLE_AACDECODER decoder = (HANDLE_AACDECODER)MAPIAACDecoder;
...
CVI_S32 AACDecoderSetEosFlag(MAPIAACDecoder MAPIAACDecoder,
								CVI_S32 s32Eosflag)
{
	//TODO:
	return 0;
 ...
 *N/A
 */
CVI_S32 AACFlushCodec(MAPIAACDecoder MAPIAACDecoder)
{
	//TODO:
	return 0;
}
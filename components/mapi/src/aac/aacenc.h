...
typedef enum {
	...
	MAPI_AACENC_OK                     = 0x0000,
	/*!< Handle passed to function call was invalid. */
	MAPI_AACENC_INVALID_HANDLE         = 0x0020,
	/*!< Memory allocation failed. */
	MAPI_AACENC_MEMORY_ERROR           = 0x0021,
	/*!< Parameter not available. */
	MAPI_AACENC_UNSUPPORTED_PARAMETER  = 0x0022,
	/*!< Configuration not provided. */
	MAPI_AACENC_INVALID_CONFIG         = 0x0023,
	/*!< General initialization error. */
	MAPI_AACENC_INIT_ERROR             = 0x0040,
	/*!< AAC library initialization error. */
	MAPI_AACENC_INIT_AAC_ERROR         = 0x0041,
	/*!< SBR library initialization error. */
	MAPI_AACENC_INIT_SBR_ERROR         = 0x0042,
	/*!< Transport library initialization error. */
	MAPI_AACENC_INIT_TP_ERROR          = 0x0043,
	/*!< Meta data library initialization error. */
	MAPI_AACENC_INIT_META_ERROR        = 0x0044,
	/*!< The encoding process was interrupted by an unexpected error. */
	MAPI_AACENC_ENCODE_ERROR           = 0x0060,
	/*!< End of file reached. */
	MAPI_AACENC_ENCODE_EOF             = 0x0080
} MAPI_AACENC_ERROR_E;
...
typedef struct mapiAACENC_VERSION_S {
	CVI_U8 aVersion[64];
} AACENC_VERSION_S;
 ...
 *N/A
 */
CVI_S32  MAPI_AACENC_GetVersion(AACENC_VERSION_S *pVersion);

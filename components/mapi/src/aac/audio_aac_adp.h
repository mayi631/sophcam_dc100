...
typedef enum mapiAAC_TRANS_TYPE_E {
...
typedef struct mapiAAC_FRAME_INFO_S {
...
typedef struct mapiAENC_ATTR_AAC_S {
...
typedef struct mapiAENC_AAC_ENCODER_S {
	AAC_ENCODER_S *pstAACState;
	AENC_ATTR_AAC_S stAACAttr;
	CVI_BOOL bRawMode;
} AENC_AAC_ENCODER_S;

typedef struct mapiADEC_ATTR_AAC_S {
	AAC_TRANS_TYPE_E enTransType;
	AUDIO_SOUND_MODE_E enSoundMode;
	AUDIO_SAMPLE_RATE_E enSmpRate;
} ADEC_ATTR_AAC_S;

typedef struct mapiADEC_AAC_DECODER_S {
	void *pstAACState;
	ADEC_ATTR_AAC_S stAACAttr;
} ADEC_AAC_DECODER_S;
#endif
CVI_S32 MAPI_MPI_AENC_AacInit(void);

CVI_S32 MAPI_MPI_AENC_AacDeInit(void);

CVI_S32 MAPI_MPI_ADEC_AacInit(void);

CVI_S32 MAPI_MPI_ADEC_AacDeInit(void);

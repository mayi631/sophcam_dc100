...
lv_obj_t *obj_sysMenu_Format_s;       // 底层窗口
lv_obj_t *obj_sysMenu_Format_Float_s; // 提示浮窗
lv_obj_t *obj_dialog_s;               // 提示内容容器
void format_key_cb(int key_code, int key_value);
void format_display_tips_anim_complete(lv_anim_t *a); // 显示提示动画
void delete_float_win(void);                          // 删除提示浮窗
static void sysmenu_format_click_callback(lv_obj_t *obj);
static void sysMenu_Delete_Complete_anim_cb(lv_anim_t* a)
{
    if (lv_obj_is_valid(obj_sysMenu_Format_Float_s)) {
        lv_obj_del(obj_sysMenu_Format_Float_s);
        obj_sysMenu_Format_Float_s = NULL;
    }
    if (lv_obj_is_valid(obj_dialog_s)) {
        obj_dialog_s = NULL;
    }
    // 取消浮窗的按键处理器
...
void sysMenu_Format_Delete_anim(void)
{
    ...
    // 设置动画完成回调（销毁对象）
    lv_anim_set_completed_cb(&Delete_anim, sysMenu_Delete_Complete_anim_cb);

    lv_anim_start(&Delete_anim);
}

//删除浮窗
void delete_float_win(void)
{
    if (lv_obj_is_valid(obj_sysMenu_Format_Float_s)) {
        lv_obj_del(obj_sysMenu_Format_Float_s);
        obj_sysMenu_Format_Float_s = NULL;
    ...
    if (!lv_obj_is_valid(obj_sysMenu_Format_s)) {
        return;
    }

    lv_obj_t* settings_cont = lv_obj_get_child(obj_sysMenu_Format_s, 1);
    ...
    init_focus_group(settings_cont, GRID_COLS, GRID_ROWS, focusable_objects, GRID_MAX_OBJECTS,
                     sysmenu_format_click_callback, target_obj);
    set_current_page_handler(handle_grid_navigation);
}

static void sysMenu_Format_btn_back_event_handler(lv_event_t *e)
{
...
static void sysMenu_Format_OK_Cancel_btn_event_handler(lv_event_t *e)
{
    ...
    case LV_EVENT_CLICKED: {
        ...
        lv_obj_t* parent = lv_obj_get_parent(btn_clicked); //获取点击事件的父控件。btn_cont
        ...
        for (uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
            if (lv_obj_get_child(parent, i) == btn_clicked) {
                // 隐藏dialog的所有控件
                for (uint8_t j = 0; j < lv_obj_get_child_cnt(obj_dialog_s); j++) {
                    if (!lv_obj_has_flag(lv_obj_get_child(obj_dialog_s, j), LV_OBJ_FLAG_HIDDEN) && j != 2 && i) {
                        lv_obj_add_flag(lv_obj_get_child(obj_dialog_s, j), LV_OBJ_FLAG_HIDDEN);
                    }
                }
                if (i == 1) {
                    if (ui_common_cardstatus()) {
                        MLOG_DBG("正在格式化......\n");
                        // 先显示"正在格式化"并刷新UI
                        lv_label_set_text(lv_obj_get_child(obj_dialog_s, 2), str_language_formatting[get_curr_language()]);
                        lv_obj_add_style(lv_obj_get_child(obj_dialog_s, 2), &ttf_font_34, LV_PART_MAIN | LV_STATE_DEFAULT);
                        lv_refr_now(lv_disp_get_default());

                        MESSAGE_S Msg = { 0 };
                        Msg.topic = EVENT_MODEMNG_CARD_FORMAT;
                        // 发送同步消息，通过 ui_event_type 判断结果
                        UICOMM_SendSyncMsg(&Msg, 20000); // 20秒超时
                        if (ui_event_type == EVT_FORMAT_SUCCESS) {
                            lv_label_set_text(lv_obj_get_child(obj_dialog_s, 2), str_language_format_successful[get_curr_language()]);
                        } else {
                            lv_label_set_text(lv_obj_get_child(obj_dialog_s, 2), str_language_format_failed[get_curr_language()]);
                        }
                    } else {
                        lv_label_set_text(lv_obj_get_child(obj_dialog_s, 2), str_language_sd_card_not_found[get_curr_language()]);
                    }
                    lv_obj_add_style(lv_obj_get_child(obj_dialog_s, 2), &ttf_font_34, LV_PART_MAIN | LV_STATE_DEFAULT);
                    lv_anim_t anim;
                    lv_anim_init(&anim);
                    ...
                    lv_anim_set_completed_cb(&anim, format_display_tips_anim_complete);
                    lv_anim_start(&anim);

                } else {
                    delete_float_win(); // 删除当前页面并进行跳转
                    break;
                }
            }
        }
...
void sysMenu_Format_Sure(void)
{
    // 创建浮层背景
    obj_sysMenu_Format_Float_s = lv_obj_create(obj_sysMenu_Format_s);
    lv_obj_remove_style_all(obj_sysMenu_Format_Float_s);
    lv_obj_set_size( obj_sysMenu_Format_Float_s , H_RES, V_RES);
    lv_obj_add_style( obj_sysMenu_Format_Float_s , &style_common_main_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(obj_sysMenu_Format_Float_s, LV_OPA_70, LV_PART_MAIN);
    lv_obj_align(obj_sysMenu_Format_Float_s, LV_ALIGN_DEFAULT, 0, 0);

    // 创建对话框容器
    obj_dialog_s = lv_obj_create(obj_sysMenu_Format_Float_s);
    lv_obj_remove_style_all(obj_dialog_s);
    lv_obj_set_size(obj_dialog_s, 500, 300);
    lv_obj_align(obj_dialog_s, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_bg_opa(obj_dialog_s, LV_OPA_100, LV_PART_MAIN);
    lv_obj_set_style_bg_color(obj_dialog_s, lv_color_hex(0x020524), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(obj_dialog_s, 20, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(obj_dialog_s, 2, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(obj_dialog_s, lv_color_hex(0x404040), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_width(obj_dialog_s, 30, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_color(obj_dialog_s, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_opa(obj_dialog_s, LV_OPA_50, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_spread(obj_dialog_s, 5, LV_PART_MAIN | LV_STATE_DEFAULT);

    // 添加警告图标
    lv_obj_t *icon = lv_label_create(obj_dialog_s);
    ...
    // 添加标题
    lv_obj_t* title = lv_label_create(obj_dialog_s);
    lv_label_set_text(title, str_language_format_warning[get_curr_language()]);
    lv_obj_add_style(title, &ttf_font_28, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(title, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 80);

    // 添加说明文本
    lv_obj_t* explain = lv_label_create(obj_dialog_s);
    // 使用多语言字符串数组
    int lang_index = get_curr_language(); // 获取当前语言索引
    char explain_text[200];
    snprintf(explain_text, sizeof(explain_text), "%s\n%s", str_language_are_you_sure_to_format[lang_index],
        str_language_data_cannot_be_recovered_after_formatting[lang_index]);
    ...
    // 创建按钮容器
    lv_obj_t *btn_cont = lv_obj_create(obj_dialog_s);
    ...
    lv_obj_add_event_cb(btn_ok, sysMenu_Format_OK_Cancel_btn_event_handler, LV_EVENT_CLICKED, NULL);
    set_current_page_handler(format_key_cb);
}

static void sysMenu_Format_Select_btn_event_handler(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    switch(code) {
        case LV_EVENT_CLICKED: {
            ...
            for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
                if(lv_obj_get_child(parent, i) == cont) {
                    ...
                    {
                        sysMenu_Format_Delete_anim(); //删除当前页面并进行跳转
                    }
                }
            }
...
static void sysmenu_format_click_callback(lv_obj_t *obj)
{
    MLOG_DBG("sysmenu_format_click_callback\n");
...
static void sysmenu_format_menu_callback(void)
{
    MLOG_DBG("sysmenu_format_menu_callback\n");
    if (lv_obj_is_valid(obj_sysMenu_Format_Float_s)) {
        lv_obj_del(obj_sysMenu_Format_Float_s);
        obj_sysMenu_Format_Float_s = NULL;
    }
...
void sysMenu_Format(lv_ui_t *ui)
{

    // 创建主页面1 容器
    if(obj_sysMenu_Format_s != NULL) {
        if(lv_obj_is_valid(obj_sysMenu_Format_s)) {
            MLOG_DBG("obj_sysMenu_Format_s 仍然有效，删除旧对象\n");
            lv_obj_del(obj_sysMenu_Format_s);
        } else {
            MLOG_DBG("obj_sysMenu_Format_s 已被自动销毁，仅重置指针\n");
        }
        obj_sysMenu_Format_s = NULL;
    }

    // Write codes resscr
    obj_sysMenu_Format_s = lv_obj_create(NULL);
    lv_obj_set_size( obj_sysMenu_Format_s , H_RES, V_RES);
    lv_obj_add_style( obj_sysMenu_Format_s , &style_common_main_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_add_event_cb(obj_sysMenu_Format_s, gesture_event_handler, LV_EVENT_GESTURE, ui);
    // Write codes cont_top (顶部栏)
    lv_obj_t *cont_top = lv_obj_create(obj_sysMenu_Format_s);
    ...
    lv_obj_add_event_cb(btn_back, sysMenu_Format_btn_back_event_handler, LV_EVENT_CLICKED, NULL);

    // Write codes title (标题)
    lv_obj_t* title = lv_label_create(cont_top);
    lv_label_set_text(title, str_language_format[get_curr_language()]);
    ...
    // 创建设置选项容器
    lv_obj_t *settings_cont = lv_obj_create(obj_sysMenu_Format_s);
    ...
    for(int i = 0; i < 2; i++) {
        ...
        lv_obj_add_event_cb(btn, sysMenu_Format_Select_btn_event_handler, LV_EVENT_ALL, btn);
    ...
    // 在上方添加一条分割线
    lv_obj_t *up_line                       = lv_line_create(obj_sysMenu_Format_s);
    ...
    lv_obj_t *target_obj = lv_obj_get_child(settings_cont, 2);
    // 初始化焦点组
    init_focus_group(settings_cont, GRID_COLS, GRID_ROWS, focusable_objects, GRID_MAX_OBJECTS, sysmenu_format_click_callback, target_obj);
    // 设置当前页面的按键处理器
    set_current_page_handler(handle_grid_navigation);
    takephoto_register_menu_callback(sysmenu_format_menu_callback);

    // Update current screen layout.
    lv_obj_update_layout(obj_sysMenu_Format_s);
}

void format_display_tips_anim_complete(lv_anim_t* a)
{
    /* 动画完成回调：删除浮窗并停止动画 */
    if (lv_obj_is_valid(obj_sysMenu_Format_Float_s)) {
        delete_float_win();
...
void format_key_cb(int key_code, int key_value)
{
    ...
    if(key_code == KEY_MENU && key_value == 1) {
        ...
        index           = 0;
        lv_obj_t *child = lv_obj_get_child(obj_sysMenu_Format_Float_s, 0);
        ...
        child = lv_obj_get_child(obj_sysMenu_Format_Float_s, 0);
        child = lv_obj_get_child(child, 3);
        child = lv_obj_get_child(child, 1);
        ...
        for(uint8_t j = 0; j < lv_obj_get_child_cnt(obj_dialog_s); j++) {
            if(!lv_obj_has_flag(lv_obj_get_child(obj_dialog_s, j), LV_OBJ_FLAG_HIDDEN) && j != 2 && index) {
                lv_obj_add_flag(lv_obj_get_child(obj_dialog_s, j), LV_OBJ_FLAG_HIDDEN);
            }
        }
        MLOG_DBG("选择%d\n", index);

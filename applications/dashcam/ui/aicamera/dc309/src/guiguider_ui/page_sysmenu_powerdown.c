...
lv_obj_t *obj_sysMenu_PowerDown_s;

static uint8_t powerDown_Current_Index_s = 0;
...
static void powerDown_win_Delete_anim(void)
{
    ...
    // 设置动画完成回调（销毁对象）
    lv_anim_set_completed_cb(&Delete_anim, powerDown_Del_Complete_anim_cb);

    lv_anim_start(&Delete_anim);
}

static void sysMenu_PowerDown_btn_back_event_handler(lv_event_t *e)
{
...
static void sysMenu_PowerDown_Select_btn_event_handler(lv_event_t *e)
{
        ...
        case LV_EVENT_CLICKED: {
            ...
            for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
                ...
                if(lv_obj_get_child(parent, i) == btn_clicked) {
                    powerDown_Current_Index_s = i;
                    // 获取按钮标签文本
                    lv_obj_t *label = lv_obj_get_child(lv_obj_get_child(parent, i), 0);
                    if(label && lv_obj_check_type(label, &lv_label_class)) {
                        const char *txt = lv_label_get_text(label);
                        if(txt) strncpy(g_sysbtn_labelPowerdown, txt, sizeof(g_sysbtn_labelPowerdown));
                    }
                    ...
                    Msg.arg1  = PARAM_MENU_POWER_OFF;
                    Msg.arg2  = powerDown_Current_Index_s / 2;
...
static void sysmenu_powerdown_click_callback(lv_obj_t *obj)
{
    MLOG_DBG("sysmenu_powerdown_click_callback\n");
...
static void sysmenu_powerdown_menu_callback(void)
{
    MLOG_DBG("sysmenu_powerdown_menu_callback\n");
    ui_load_scr_animation(&g_ui, &obj_sysMenu_Setting_s, 1, NULL, sysMenu_Setting, LV_SCR_LOAD_ANIM_NONE, 0, 0,
        false, true);
}
...
void sysMenu_PowerDown(lv_ui_t *ui)
{
    ...
    if(obj_sysMenu_PowerDown_s != NULL) {
        if(lv_obj_is_valid(obj_sysMenu_PowerDown_s)) {
            MLOG_DBG("obj_sysMenu_PowerDown_s 仍然有效，删除旧对象\n");
            lv_obj_del(obj_sysMenu_PowerDown_s);
        } else {
            MLOG_DBG("obj_sysMenu_PowerDown_s 已被自动销毁，仅重置指针\n");
        }
        obj_sysMenu_PowerDown_s = NULL;
    }

    // Write codes resscr
    obj_sysMenu_PowerDown_s = lv_obj_create(NULL);
    lv_obj_set_size( obj_sysMenu_PowerDown_s , H_RES, V_RES);
    lv_obj_add_style( obj_sysMenu_PowerDown_s , &style_common_main_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_add_event_cb(obj_sysMenu_PowerDown_s, gesture_event_handler, LV_EVENT_GESTURE, ui);

    // Write codes cont_top (顶部栏)
    lv_obj_t *cont_top = lv_obj_create(obj_sysMenu_PowerDown_s);
    ...
    lv_obj_add_event_cb(btn_back, sysMenu_PowerDown_btn_back_event_handler, LV_EVENT_CLICKED, NULL);

    // Write codes title (标题)
    lv_obj_t* title = lv_label_create(cont_top);
    lv_label_set_text(title, str_language_auto_shutdown[get_curr_language()]);
    ...
    // 创建设置选项容器
    lv_obj_t *settings_cont = lv_obj_create(obj_sysMenu_PowerDown_s);
    ...
    // 创建分辨率选项按钮
    const char *btn_labels[] = {str_language_off[get_curr_language()], str_language_3_minutes[get_curr_language()], str_language_5_minutes[get_curr_language()], str_language_10_minutes[get_curr_language()]};
    ...
    for(int i = 0; i < 4; i++) {
        ...
        lv_obj_add_event_cb(btn, sysMenu_PowerDown_Select_btn_event_handler, LV_EVENT_ALL, settings_cont);

        if(i == powerDown_Current_Index_s / 2) {
            {
    ...
    //先设置焦点控件,再进行滚动,否则会直接滚动到最下,不知什么原因.
    //获取焦点控件
    lv_obj_t *chlid = lv_obj_get_child(settings_cont, powerDown_Current_Index_s);
    lv_group_focus_obj(chlid);
    lv_obj_add_state(chlid, LV_STATE_FOCUS_KEY);
    lv_obj_scroll_to_y(settings_cont, ((powerDown_Current_Index_s / 2) * MENU_BTN_SIZE), LV_ANIM_OFF);
    // //设置焦点渐变
    // lv_set_obj_grad_style(chlid, LV_GRAD_DIR_VER, lv_color_hex(0xFBDEBD), lv_color_hex(0xF09F20));
    // //设置焦点BG
    // lv_obj_set_style_bg_color(chlid, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    // 在上方添加一条分割线
    lv_obj_t *up_line                       = lv_line_create(obj_sysMenu_PowerDown_s);
    ...
    lv_obj_t *target_obj = lv_obj_get_child(settings_cont, powerDown_Current_Index_s);
    // 初始化焦点组
    init_focus_group(settings_cont, GRID_COLS, GRID_ROWS, focusable_objects, GRID_MAX_OBJECTS, sysmenu_powerdown_click_callback, target_obj);
    // 设置当前页面的按键处理器
    set_current_page_handler(handle_grid_navigation);
    takephoto_register_menu_callback(sysmenu_powerdown_menu_callback);

    // Update current screen layout.
    lv_obj_update_layout(obj_sysMenu_PowerDown_s);
